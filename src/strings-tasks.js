...
function concatenateStrings(value1, value2) {
  const newstr = '';
  return newstr.concat(value1, value2);
}
...
function isPalindrome(str) {
  ...
  const lowStr2 = lowStr.replace(/[\s.,%,!,?]/g, '');
  if (lowStr2 === lowStr2.split('').reverse().join('')) {
    return true;
  }
  return false;
}
 ...
 * Please do not use concatenation
...
function extractNameFromTemplate(value) {
  // Находим индексы начала и конца имени
  const startIndex = value.indexOf('Hello, ') + 'Hello, '.length;
  const endIndex = value.indexOf('!');

  // Извлекаем имя из строки
  const extractedName = value.substring(startIndex, endIndex);

  return extractedName.trim(); // Убираем возможные пробелы в начале и конце
}
